...
func shoot_at_player():
	...
	# 2. 화면에 추가 (이게 global_position 설정부다 먼저 와야 합니다!)
...
func _on_death():
	# 자폭 패턴 구현
	var num_bullets = randi() % 3 + 5 # 5에서 7발 생성
	for i in range(num_bullets):
		var bullet = bullet_scene.instantiate()
		var random_offset = Vector2(randf_range(-30, 30), randf_range(-30, 30))
		bullet.global_position = global_position + random_offset
		
		# 마지막 방향에 약간의 랜덤 오프셋 추가
		var direction_offset = randf_range(-0.2, 0.2)
		bullet.direction = (transform.x.rotated(direction_offset)).normalized()
